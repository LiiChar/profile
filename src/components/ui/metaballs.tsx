...
const MetaBalls: React.FC<MetaBallsProps> = ({
	...
	useEffect(() => {
		...
		for (let i = 0; i < Math.min(ballCount, 50); i++) {
			const h1 = hash31(i + 1);
		...
		function resize() {
			...
			program.uniforms.iResolution.value.set(scaledWidth, scaledHeight, 0);
		}
	...
	return (
		<div
			...
			ref={containerRef}
			className={cn(
				'w-full h-full relative pointer-events-none bg-transparent',
				attr.className
			)}
		/>
	);
