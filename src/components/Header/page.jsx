...
const Header = memo(function Header() {
	...
	const handleScroll = React.useCallback(e => {
		...
		const viewUser =
			(e.target.pageYOffset || e.target.documentElement.scrollTop) -
			(e.target.documentElement.clientTop || 0);
				...
				<div className='links'>
					<div className='wrapHeader'>
						<div
							...
							className={`link ${hash == '#home' ? 'pickLink' : 'unPickLink'}`}
						>
							HOME
						</div>
						<div
							...
							className={`link ${
								hash == '#portfolio' ? 'pickLink' : 'unPickLink'
							}`}
						>
							PORTFOLIO
						</div>
						<div
							...
							className={`link ${
								hash == '#contacts' ? 'pickLink' : 'unPickLink'
							}`}
						>
							CONTACTS
						</div>
